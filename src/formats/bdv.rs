...
//!
//! This is a faithful port of `loci.formats.in.BDVReader` in Java's default
//! flattened-resolution mode. Internal state still groups mipmap levels under a
//! logical `ViewSetup`, but the public series map exposes one series per
...
//!
//! Each public series reports `resolution_count() == 1`. Image names follow
//! Java's `P_t{first:05}, W_s{setup:02}_{level}` pattern.
//!
...
/// One resolution level of one logical BDV setup. `meta.resolution_count` is
/// baked in at construction time (1 when `flattened_resolutions`, else the
/// setup's true level count), since `flattened_resolutions` can't change
/// after `set_id`.
#[derive(Clone)]
struct ResolutionInfo {
    /// Resolution level (the `{level}` group number).
    level: u32,
    /// Core metadata for this resolution.
    meta: ImageMetadata,
}
...
/// level becomes its own public series (`public_series` maps one entry per
/// (series, resolution) pair). When false, one public series per logical
/// setup, with levels reachable via `resolution_count()`/`set_resolution()`.
#[derive(Clone)]
struct SeriesInfo {
    ...
    timepoints: Vec<u32>,
    /// Nested resolution levels for this setup, sorted ascending by level
    /// number (index 0 = full resolution).
...
pub struct BdvReader {
    ...
    /// Logical series list (one per logical setup).
    series: Vec<SeriesInfo>,
    /// Public series map: `(logical series, resolution)`. When
    /// `flattened_resolutions` is true, one entry per (series, resolution)
    /// pair; when false, one entry per series (resolution always 0).
    public_series: Vec<(usize, usize)>,
    current_public_series: usize,
    current_series: usize,
    /// Active resolution level within the current series (index into
    /// `series[current_series].resolutions`).
...
impl BdvReader {
    ...
    fn image_data_path(&self, no: u32) -> Result<String> {
        ...
        Ok(format!(
            "t{:05}/s{:02}/{}/cells",
            timepoint, setup, ri.level
        ))
    }

    fn current_resolution_info(&self) -> Option<&ResolutionInfo> {
        self.series
            .get(self.current_series)?
            .resolutions
            .get(self.current_resolution)
    ...
    fn current_metadata(&self) -> Option<&ImageMetadata> {
        self.current_resolution_info().map(|r| &r.meta)
    }

    fn rebuild_public_series(&mut self) {
        self.public_series.clear();
        for (series_index, series) in self.series.iter().enumerate() {
            if self.flattened_resolutions {
                for resolution_index in 0..series.resolutions.len().max(1) {
                    self.public_series.push((series_index, resolution_index));
                }
            } else {
...
impl FormatReader for BdvReader {
    ...
    fn set_series(&mut self, s: usize) -> Result<()> {
        ...
        let Some((logical_series, resolution)) = self.public_series.get(s).copied() else {
            return Err(BioFormatsError::SeriesOutOfRange(s));
        };
        self.current_public_series = s;
        self.current_series = logical_series;
    ...
    fn set_resolution(&mut self, level: usize) -> Result<()> {
        ...
        if self.flattened_resolutions {
            ...
            let si = self
                ...
                .get(self.current_series)
                .ok_or(BioFormatsError::NotInitialized)?;
            if level >= si.resolutions.len() {
                return Err(BioFormatsError::Format(format!(
                    "resolution {level} out of range (max {})",
                    si.resolutions.len().saturating_sub(1)
                )));
            }
    ...
    fn ome_metadata(&self) -> Option<OmeMetadata> {
        ...
        for &(series_index, resolution_index) in &self.public_series {
            let si = self.series.get(series_index)?;
            let ri = si.resolutions.get(resolution_index)?;
            ...
            let name = match setup {
                Some(s) => format!("P_t{first_timepoint:05}, W_s{:02}_{}", s.id, ri.level),
                None => format!("P_t{first_timepoint:05}, W_s{:02}_{}", si.setup, ri.level),
            };
            let (psx, psy, psz) = match si.voxel_size {
                Some((x, y, z)) => (Some(x), Some(y), Some(z)),
                None => (None, None, None),
            };
            let base_meta = &ri.meta;
            let channels: Vec<OmeChannel> = (0..base_meta.size_c.max(1))
                .map(|_| OmeChannel {
                    samples_per_pixel: 1,
                    ..Default::default()
                })
...
impl BdvReader {
    ...
    fn parse_structure(&mut self) -> Result<()> {
        ...
        // Build the logical series list. Only first-timepoint setup coordinates
        // that carry at least one `cells` dataset become series; other
        // timepoints are resolved at plane-read time through T.
        ...
        for setup in &self.setup_attribute_list {
            ...
            for &level in &levels {
                ...
                resolutions.push(ResolutionInfo { level, meta });
            }
            if !resolutions.is_empty() {
                // Java's `setFlattenedResolutions(false)`: report the true
                // level count on every resolution's metadata instead of the
                // flattened default of 1. The toggle can't change after
                // set_id, so this is safe to bake in now.
                if !self.flattened_resolutions {
                    let level_count = resolutions.len() as u32;
                    for ri in &mut resolutions {
                        ri.meta.resolution_count = level_count;
                    }
                }
...
mod tests {
  ...
  <TIMEPOINTS type="pattern"><INTEGERPATTERN>1-5:2</INTEGERPATTERN></TIMEPOINTS>
    ...
    fn bdv_plane_index_maps_xyz_tc_to_timepoint_and_channel_setup() {
        ...
        r.rebuild_public_series();

        assert_eq!(r.series_count(), 2);
        assert_eq!(r.resolution_count(), 1);
        ...
        assert_eq!(r.image_data_path(0).unwrap(), "t00003/s10/4/cells");
        assert_eq!((r.metadata().size_x, r.metadata().size_y), (2, 2));
    }
  ...
  <SequenceDescription>
    ...
    fn bdv_fixture_keeps_timepoints_in_size_t_not_series() {
        ...
        let full_size = (r.metadata().size_x, r.metadata().size_y);
        r.set_series(1).unwrap();
        assert_eq!(r.resolution(), 0);
        assert!(r.metadata().size_x <= full_size.0);
        assert!(r.metadata().size_y <= full_size.1);
    }
}
